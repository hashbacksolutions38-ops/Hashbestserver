...
const DATABASE_PUBLIC_URL = process.env.DATABASE_PUBLIC_URL || process.env.RAILWAY_DATABASE_URL;
const CORS_ORIGIN = process.env.CORS_ORIGIN || "*";

const pool = DATABASE_PUBLIC_URL
  ? new Pool({
...
async function dbQuery(textQuery, values = []) {
  if (!pool) {
    const error = new Error("DATABASE_PUBLIC_URL is not configured. Connect the Railway PostgreSQL database first.");
...
async function initDatabase() {
  if (!pool) {
    databaseError = "DATABASE_PUBLIC_URL is not configured";
    return;
  }
  try {
